...
const server = 'https://enzo-customers.herokuapp.com/api'
...
let initHome = () => {
	...
	let vm = new Vue({
		...
		beforeMount() {
			...
			.then(results => {
				let customers = results.data.customers
	...
	let vm = new Vue({
				...
				metafields.forEach(mf => {
					if (mf.key == 'samples') {
						...
						if (self[key].fields !== undefined && self[key].fields !== null) {
							for (let fieldKey in self[key].fields) {
								arr.forEach(arrItem => {
									// console.log(arrItem, fieldKey)
		...
		methods: {
			...
			update() {
				...
				axios.post(`${server}/customers/update`, obj)
	...
	let vm = new Vue({
		...
		methods: {
			...
			create() {
				...
				axios.post(`${server}/customers/create`, obj)
				.then(function(response) {
					console.log(response.data)
